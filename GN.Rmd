---
title: "Introduction to Scientific Programming and Simulation 2017"
...
The code from our catch-up-session can be found in file `Rintro.R`,
the transcript including in- and output is here: `Rintro.Rt`.
...
atomic vector (corresponding `NA`)
  ...
  + logical (`NA`)
  + integer (`NA_integer_`) - special case `factor` (attributes)
...
typeof, (mode), length / dim, attributes, class

getwd / setwd

Data I/O: `read.csv`, `write.csv`, `file.path`

help / ?

ls / rm / library

comments #, case sensitive, <-, ==, +, letters
...
The code from our data.table session can be found in file `datatable.R`.
As usual, the transcript including in- and output is here: `datatable.Rt`.
...
you could use the path to this file within `render`, or set the working directory to the directory, where the source file is stored.


Example of a presentation:

https://github.com/jimhester/presentations/tree/master/2017_01_13-RStudio_conf-Database_Best_Practices
...
If `devtools::install_github` doesn't work (Could not find build
tools necessary to build ...), try
```{r mac_devtools, eval=FALSE}
options(buildtools.check = function(action) TRUE)
```
See also https://stackoverflow.com/questions/38725060/error-could-not-find-build-tools-necessary-to-build-stringi-when-trying-devto
